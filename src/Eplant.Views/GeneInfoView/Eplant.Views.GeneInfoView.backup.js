(function() {
  ...
  */
  ...
  /* Feature */
  Eplant.Views.GeneInfoView.subfeatures = null;  // Subfeatures
          ...
          $.ajax({
            ...
            success: $.proxy(function(data) {
              ...
              for(var i =0;i<data.features.length;i++){
                ...
                Eplant.Views.GeneInfoView.subfeatures = subfeatures;
              ...
              $.ajax({
                ...
                success: $.proxy(function(data) {
                  ...
                  var originalSeq = data.result[0].sequence;
                  var finalSeq = originalSeq;
                  finalSeq = '<span style="color:#9966ff;">' + finalSeq + '</span>';
                  
                  // Add formating
                  for (var i = 0; i < subfeatures.length; i++) {
                    if (subfeatures[i].type == 'exon') {
                      var x = subfeatures[i].start - this.chromosome_start + 1;
                      var y = subfeatures[i].end - this.chromosome_start + 1;
                  ...
                  for (var i = 0; i < subfeatures.length; i++) {
                    if (subfeatures[i].type == 'five_prime_UTR') {
                      var x = subfeatures[i].start - this.chromosome_start + 1;
                      var y = subfeatures[i].end - this.chromosome_start + 1;
                      if (subfeatures[i].strand == 1) {
                    ...
                    if (subfeatures[i].type == 'three_prime_UTR') {
                      var x = subfeatures[i].start - this.chromosome_start + 1;
                      var y = subfeatures[i].end - this.chromosome_start + 1;
                      if (subfeatures[i].strand == 1) {
                        var newSeq = originalSeq.substring(x-3, y);
      ...
      $.ajax({
        ...
        success: $.proxy(function(summary) {
          if(summary.result&&summary.result.length>0){
            ...
            var seqString = summary.result[0].sequence;
