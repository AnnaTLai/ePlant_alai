(function() {
	...
	*/
	...
	Eplant.Views.MoleculeView.prototype.generateCitation = function(template,eValue,pdbid) {
		...
		if(template&&eValue){
			this.citation += 'The 3D molecule data come from Phyre2: Kelley LA et al. Nature Protocols 10, 845-858 (2015). The PDB template used to model this structure was '+template+' and the HMMPred e-value was '+eValue+'. <br><br>SNP data come from The 1001 Genomes Consortium (2016) 1135 genomes reveal the global pattern of polymorphism in Arabidopsis thaliana Cell, Volume 166, Issue 2, 481-491 and the Polymorph API at https://tools.1001genomes.org/polymorph/<br><br>Pfam domain data come from Finn et al., (2014). CDD feature hits come from Marchler-Bauer et al., (2015).<br><br> This viewer was built with JSmol.';
		}else if(pdbid){
			this.citation += 'The 3D molecule data come from the PDB, <a href="http://www.rcsb.org/pdb/explore/explore.do?structureId='+pdbid+'" target="_blank">doi: 10.2210/pdb'+pdbid+'/pdb</a>. The water molecules were removed from the PDB files.<br><br>SNP data come from The 1001 Genomes Consortium (2016) 1135 genomes reveal the global pattern of polymorphism in Arabidopsis thaliana Cell, Volume 166, Issue 2, 481-491 and the Polymorph API at https://tools.1001genomes.org/polymorph/<br><br>Pfam domain data come from Finn et al., (2014). CDD feature hits come from Marchler-Bauer et al., (2015).<br><br> This viewer was built with JSmol.';
		}else{
			this.citation += 'The 3D molecule data come from Phyre2: Kelley LA et al. Nature Protocols 10, 845-858 (2015). <br><br>SNP data come from The 1001 Genomes Consortium (2016) 1135 genomes reveal the global pattern of polymorphism in Arabidopsis thaliana Cell, Volume 166, Issue 2, 481-491 and the Polymorph API at https://tools.1001genomes.org/polymorph/<br><br>Pfam domain data come from Finn et al., (2014). CDD feature hits come from Marchler-Bauer et al., (2015).<br><br> This viewer was built with JSmol.';
		}
			...
			$.getJSON(Eplant.ServiceUrl + 'JSMol.cgi?agi=' + this.geneticElement.identifier, $.proxy(function(response) {
				...
				if (response.link != "") {
					...
					$.get( response.link, $.proxy(function( raw ) {
						...
						$.ajax({
							url: '//bar.utoronto.ca/webservices/bar_araport/get_protein_sequence_by_identifier.php?locus='+this.geneticElement.identifier+'.1',
							type: 'GET',
							timeout: 50000,
