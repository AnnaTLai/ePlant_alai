...
BEN CHANGED:
	...
	refactored from cytoscape 2 to cytoscape 3

	event.cyTarget => event.target
...
(function () {
	...
	Eplant.Views.InteractionView = function (geneticElement) {
		...
		this.intLoader.then((function (data) {
			...
			this.subLoader.then((function (data) {
				...
				this.cyConf["container"] = $(this.domContainer);
				cytoscape(this.cyConf);
				console.trace(this.cyConf);
				//cytoscape 2.4 equivalent of the initialization above
		...
		//add view title to top of screen
		if (this.name) {
			$(this.labelDom).empty();
	...
	Eplant.Views.InteractionView.viewName = 'InteractionView'; //BEN CHANGED
	...
	Eplant.Views.InteractionView.prototype.downloadRawData = function () {
		let dataString = '//bar.utoronto.ca/eplant/cgi-bin/get_interactions_dapseq.py?locus=' + this.geneticElement.identifier;
	...
	Eplant.Views.InteractionView.prototype.setCyConf = function () {
		...
		// Ready event handler
		this.cyConf.ready = $.proxy(function (event) {
			//BEN CHANGED
			// Save Cytoscape 
			this.cy = event.cy
			...
			if (!this.loadFlags.empty) {
				...
				this.cy.nodes('#noInteractionLabel').position({
					x: 0,
					y: 100
				});
				this.loadFinish();
	...
	Eplant.Views.InteractionView.prototype.setCytoscapeStyles = function () {
		//BEN CHANGED (some of the specific values, like width which were causing text clipping)
	...
	Eplant.Views.InteractionView.prototype.edgeEventHandler = function () {
		...
		this.cy.on('mouseover', 'edge', $.proxy(function (event) {
			...
			_this.edgeStationaryTimer = setTimeout(function () {
				...
				currIndex = event.target._private.index;
			}, 350);
		}, this));
		...
		this.cy.on('mousemove', 'edge', $.proxy(function (event) {
			...
			if (this.tooltip && event.target._private.index === currIndex) {
			...
			function (event, eventData, listenerData) {
				...
				if (listenerData.interactionView.geneticElement.species === event.target.species) {
					...
					if (listenerData.interactionView.cy) {
						...
						var loadedNodes = listenerData.interactionView.cy.nodes('[id ^= "' + event.target.identifier.toUpperCase() + '"]');
						...
						for (var n = 0; n < loadedNodes.length; n = n + 1) {
							...
							// Create annotation
							var annotation = new Eplant.Views.InteractionView.Annotation(event.target, listenerData.interactionView);
