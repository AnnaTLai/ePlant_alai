...
BEN CHANGED:
	...
	this.cy.$() => this.cy.nodes()

	added a .run() to the layouts

	swapped arbor to also use cose bilkent

	swapped some cytoscape parameter orders
*/
...
(function () {
	...
	Eplant.Views.InteractionView.Layout = function (interactionView, cb) {
		...
		if (!this.loadFlags.empty) {
			if (this.cy.nodes('[id $= "PROTEIN_NODE"]').length > 0) {
	...
	Eplant.Views.InteractionView.Layout.prototype.positionProtein = function () {
		// Get nodes
		var proteinNodes = this.cy.nodes('[id $= "PROTEIN_NODE"]');
		var that = this;
		if (this.loadFlags.existsPDI || proteinNodes.length < 100) {
			// Layout proteins for PDI
			var layout = proteinNodes.layout({
				name: 'cose-bilkent',
