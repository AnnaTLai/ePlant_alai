	...
	private function execute($mysqli) {
		...
		for ($i = 0; $i < count($this->JSONData); $i++) {
			if (isset($this->JSONData[$i]["agi"])) {
				...
				// Run the first query
				if ($interactors = $mysqli->query("SELECT Protein1, Protein2, Quality, Pcc, Bind_id, aiv_index FROM interactions_eplant_v4 WHERE Protein1 = '$gene'")) {
				...
				if ($getRecursive) {
					...
					$query = "SELECT Protein1, Protein2, Quality, Pcc, Bind_id, aiv_index FROM interactions_eplant_v4 WHERE (Protein1='" . $gene . "' AND Protein2='" . $gene . "')";
